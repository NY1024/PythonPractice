...
def printContacts(skypeDB):
    conn = ssqlite3.connect(skypeDB)
    c = conn.cursor()
    c.execute("SELECT displayname,skypename,city,country,phone_mobile,bithday FROM Contacts;")
    for row in c:
        print '\n[*] -- Found Contact'
        print '[+] User     :'+str(row[0])
        print '[+] Skype Username:' + str(row[1])
...
def printCallLpg(skypeDB):
    conn = sqlite3.connect(skypeDB)
    c = conn.cursor()
    c.execute("SELECT datetime(begin_timestamp,'unixepoch',identify FROM calls,conversations WHERE calls.conv_bdid = conversations.id;")
    print '\n[*] --Found Calls--'
    for row in c:
...
def printMessages(skypeDB):
    conn = sqlite3.connect(skypeDB)
    c= conn.cursor()
    c.execute("SELECT datatime(timestamp,'unixepoch'),dialog_partner,author,body_xml FROM Messages;")
    print '\n[*] --Found Messages--'
    for row in c:
        try:
